...
function draw() {
    ...
    //to check if gamestate is play
    if (gameState === PLAY) {

        //to give velocity
        jungle.velocityX = -3

        //to repeat jungle infinitly
        if (jungle.x < 100) {
            jungle.x = 400
        }

        //to jump the kangaroo
        ...
        //to reset the game when mouse is pressed over the restart image
    ...
    //to check if the score is 5 and display text
    if (score >= 5) {
        kangaroo.visible = false;
        textSize(30);
        stroke(3);
        fill("black")
        text("Congratuations!! You win the game!!", 100, 200)
        gameState = WIN;
    }

    if (gameState === WIN) {

        //set velocity of each game to 0
        jungle.velocityX = 0;
        kangaroo.velocityY = 0;
